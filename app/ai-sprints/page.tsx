...
export const metadata: Metadata = {
  ...
  description:
    "Six two-week solo sprints for AI-first roles: comps pipelines, strategy prototypes, shipped features with evals, domain RAG, regulatory trackers, and audited clinical extraction.",
};
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 pb-24">
        ...
        <header className="rounded-3xl border border-border/70 bg-card p-8 shadow-sm sm:p-10">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline" className="rounded-2xl">
              <a href="/api/ai-sprints-guide" rel="noreferrer" target="_blank">
                Open raw HTML
              </a>
            </Button>
          </div>
        ...
        {AI_SPRINTS.map((sprint) => (
          ...
          >
            ...
            <h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">{sprint.title}</h2>
            <p className="mt-1 text-lg italic text-muted-foreground">{sprint.subtitle}</p>
            <p className="mt-5 text-sm leading-relaxed text-foreground sm:text-base">
              <span className="font-semibold text-primary">The bet.</span> {sprint.bet}
            </p>
            <div className="mt-5 space-y-4 text-sm leading-relaxed text-foreground sm:text-base">
              <p>
                <span className="font-semibold text-primary">Week 1.</span> {sprint.week1}
              </p>
              <p>
                <span className="font-semibold text-primary">Week 2.</span> {sprint.week2}
              </p>
            </div>
            ...
            <blockquote className="mt-8 border-l-4 border-primary bg-primary/5 px-5 py-4 text-sm font-medium leading-relaxed text-foreground sm:text-base">
              Outcomes you can claim: &ldquo;{sprint.outcomes}&rdquo;
            </blockquote>
          </article>
        ))}

        <section className="rounded-3xl border border-border/70 bg-card p-8 shadow-sm sm:p-10">
          <h2 className="text-xl font-bold tracking-tight text-foreground">{AI_SPRINTS_PATTERN.title}</h2>
          <div className="mt-5 overflow-hidden rounded-2xl border border-border">
            <table className="w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/80">
              ...
              <tbody>
                {AI_SPRINTS_PATTERN.rows.map((row) => (
          ...
          <p className="mt-6 text-sm leading-relaxed text-muted-foreground">{AI_SPRINTS_PATTERN.closing}</p>
        </section>
      </div>
    </main>
  );
